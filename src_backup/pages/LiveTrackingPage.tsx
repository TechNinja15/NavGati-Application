...
export default function LiveTrackingPage() {
    ...
    if (loading) return <div className="p-8 text-center">Loading live details...</div>
    if (!busData) return <div className="p-8 text-center text-red-500">Bus not found or offline.</div>

    // Parse Route Data
    const routeStops = busData.bus_route_data?.stops || []
    const currentBusStops = routeStops.map((stop: string, index: number) => ({
        name: stop,
        ...
        <div className="min-h-screen bg-background flex flex-col pb-20">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <Card className="overflow-hidden shadow-card border-none">
                    <ErrorBoundary>
                        <RouteMap stops={currentBusStops.map((s: any) => s.name)} />
                    </ErrorBoundary>
                </Card>
