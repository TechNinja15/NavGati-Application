...
export default function DriverDashboard() {
  ...
  const currentBusStops = routeStops.length > 0 ? routeStops.map((stop: string, index: number) => ({
    name: stop,
    ...
    ...((Array.isArray(driverDetails?.middleStops) ? driverDetails.middleStops : []).map((stop: string, index: number) => ({
  ...
  const moveToNextStop = () => {
    if (currentStopIndex < currentBusStops.length - 1) {
      ...
      toast({
        title: "Stop Updated",
        description: `Now at ${currentBusStops[currentStopIndex + 1].name}`,
      })
    }
  }
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="relative pl-4 border-l-2 border-dashed border-muted ml-2 space-y-6 pb-2">
              {currentBusStops.map((stop: any, index: number) => (
                <div key={index} className="relative flex items-center justify-between pl-4">
                  {/* Timeline Dot */}
        ...
        <div className="p-4 bg-background border-t">
          <Button
            ...
            size="lg"
            disabled={currentBusStops.length === 0}
          >
  ...
  return (
    <div className="min-h-screen bg-background p-3 sm:p-4 space-y-4 sm:space-y-6 max-w-md mx-auto pb-20">
      {/* Header */}
      ...
      <Card className="p-4 sm:p-6 shadow-card">
        ...
        <div className="space-y-2 sm:space-y-3">
          {currentBusStops.map((stop, index) => (
            <div key={index} className={`flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 rounded-lg border ${index === currentStopIndex
              ...
              {index === currentStopIndex && currentStopIndex < currentBusStops.length - 1 && (
