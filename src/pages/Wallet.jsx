...
export default function Wallet() {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <Card className="p-6 shadow-float bg-gradient-primary text-primary-foreground">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="secondary" onClick={() => setShowRecharge(!showRecharge)}>
        ...
        {/* Recharge Section */}
        {showRecharge && (<Card className="p-4 shadow-card bg-gradient-card">
            <h3 className="font-semibold mb-4">{t("wallet.recharge_wallet")}</h3>
            <div className="space-y-4">
              <div>
                <Label htmlFor="amount">{t("wallet.enter_amount")}</Label>
                <Input id="amount" type="number" placeholder="0.00" value={rechargeAmount} onChange={(e) => setRechargeAmount(e.target.value)} className="mt-1"/>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-2">{t("wallet.quick_amounts")}</p>
                <div className="grid grid-cols-4 gap-2">
                  {quickAmounts.map((amount) => (<Button key={amount} variant="outline" size="sm" onClick={() => setRechargeAmount(amount.toString())} className="text-xs">
                      ₹{amount}
                    </Button>))}
                </div>
              </div>
              <div className="flex space-x-2">
                ...
                <Button variant="outline" onClick={() => setShowRecharge(false)} className="flex-1">
                  {t("cancel")}
                </Button>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {offers.map((offer, index) => (<Card key={index} className="p-4 shadow-card bg-gradient-card border-l-4 border-l-primary">
                <div className="flex items-center justify-between">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            return (<Card key={transaction.id} className="p-4 shadow-card bg-gradient-card">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="text-right">
                      ...
                      <Badge variant={transaction.status === 'completed' ? 'secondary' : 'destructive'} className="text-xs">
                        {transaction.status}
                      </Badge>
                    </div>
                  </div>
