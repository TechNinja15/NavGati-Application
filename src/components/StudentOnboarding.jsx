      ...
      <Card className="w-full max-w-lg p-8 shadow-card bg-gradient-card">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.city} onValueChange={(value) => setFormData(prev => ({ ...prev, city: value, institution: '' }))} disabled={!formData.state}>
              <SelectTrigger>
                <SelectValue placeholder="Select your city"/>
              </SelectTrigger>
              <SelectContent>
                {formData.state && cities[formData.state]?.map(city => (<SelectItem key={city} value={city}>{city}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.institution} onValueChange={(value) => setFormData(prev => ({ ...prev, institution: value }))} disabled={!formData.city}>
              <SelectTrigger>
                <SelectValue placeholder="Select your institution"/>
              </SelectTrigger>
              <SelectContent>
                {formData.city && institutions[formData.city]?.map(institution => (<SelectItem key={institution} value={institution}>{institution}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input id="studentBusId" type="text" placeholder="Enter your student bus ID (e.g., SB001, COL123)" value={formData.studentBusId} onChange={(e) => setFormData(prev => ({ ...prev, studentBusId: e.target.value }))} required/>
            <p className="text-xs text-muted-foreground">
              This ID is provided by your institution for bus services
            </p>
          </div>

          <Button type="submit" className="w-full" disabled={!formData.state || !formData.city || !formData.institution || !formData.studentBusId}>
