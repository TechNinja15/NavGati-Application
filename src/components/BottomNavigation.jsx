  ...
  return (<div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border">
    <nav className="flex items-center justify-around py-2 px-1 max-w-md mx-auto">
      {navigation.map((item) => {
        ...
        return (<Link key={item.name} to={item.href} className={cn("flex flex-col items-center justify-center py-2 px-3 rounded-xl text-xs font-medium transition-all duration-200", isActive
