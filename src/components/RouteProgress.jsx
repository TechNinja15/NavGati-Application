...
export default function RouteProgress({ routeId, routeName, onBack, onViewMap }) {
    const getSeatAvailability = (routeId) => {
        const seatData = {
            // Bangalore Routes
    ...
    const getRouteStops = (routeId, routeName) => {
    ...
    const [stops] = useState(getRouteStops(routeId, routeName));
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-6">
        ...
        <div className="space-y-4 mb-8">
          {stops.map((stop, index) => (<div key={index} className="flex items-center space-x-4">
              {/* Stop Icon */}
              <div className="flex flex-col items-center">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${stop.status === 'departed'
                ...
                {index < stops.length - 1 && (<div className={`w-0.5 h-8 ${stop.status === 'departed' ? 'bg-success' : 'bg-muted'}`}/>)}
              </div>
              ...
              <div className="flex-1 flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className={`font-semibold ${stop.status === 'current' ? 'text-primary' : ''}`}>
                      {stop.name}
                    </h3>
                    {stop.arrivalTime && (<span className="text-sm font-medium text-muted-foreground">
                        {stop.arrivalTime}
                      </span>)}
                  </div>
                  <div className="flex items-center justify-between mt-1">
                    ...
                    {stop.traffic && (<div className="flex items-center space-x-1">
                        <div className={`w-2 h-2 rounded-full ${stop.traffic === 'Light' ? 'bg-success' :
                    stop.traffic === 'Moderate' ? 'bg-warning' : 'bg-destructive'}`}/>
                        <span className="text-xs text-muted-foreground">{stop.traffic}</span>
                      </div>)}
                  </div>
                </div>
                {stop.status === 'current' && (<Badge variant="secondary" className="bg-primary/10 text-primary ml-2">
                    Bus Here
                  </Badge>)}
              </div>
            </div>))}
