...
export function LanguageSelector({ asMenuItem = false, currentLanguageDisplay }) {
    ...
    const handleCreateSelection = (code) => {
        setLanguage(code);
        if (asMenuItem) {
    ...
    if (asMenuItem && !isOpen) {
        return (<div className="flex items-center justify-between" onClick={() => setIsOpen(true)}>
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <Card className="w-full max-w-md p-6 space-y-6 shadow-float border-primary/20 bg-card relative">
        {asMenuItem && (<Button variant="ghost" size="icon" className="absolute right-2 top-2" onClick={() => setIsOpen(false)}>
            <X className="h-4 w-4"/>
          </Button>)}
        ...
        <div className="space-y-3">
          {languages.map((lang) => (<div key={lang.code} onClick={() => handleCreateSelection(lang.code)} className={`
              ...
              {language === lang.code && (<div className="h-6 w-6 bg-primary rounded-full flex items-center justify-center text-primary-foreground animate-in zoom-in duration-200">
                  <Check className="h-4 w-4"/>
                </div>)}
            </div>))}
        </div>

        <Button onClick={handleContinue} className="w-full h-12 text-lg font-medium shadow-lg hover:shadow-xl transition-all">
