...
export default function RouteMap({ stops }: RouteMapProps) {
    ...
    const generateMockCoordinates = (stopNames: string[]) => {
        let baseLat = 21.2514
        let baseLng = 81.6296
    ...
    useEffect(() => {
        ...
        if (map && layerGroup) {
            ...
            const latLngs = mapStops.map(s => [s.lat, s.lng] as [number, number])
    ...
    return (
        <div className="h-64 w-full rounded-lg overflow-hidden shadow-card border border-border z-0 relative">
